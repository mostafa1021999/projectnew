...
class CategoryRepo{
  ...
  Future<Response> getCategoryList(int offset, int limit) async {
    return await apiClient.getData('${AppConstants.getCategoryListUri}?limit=$limit&offset=$offset');
  }

  Future<Response> getCategoryWiseProductList(int? categoryId) async {
    return await apiClient.getData('${AppConstants.categoriesProductUri}?category_id=$categoryId');
  }
  ...
  Future<Response> getSubCategoryList(int offset, int? categoryId) async {
    return await apiClient.getData('${AppConstants.getSubCategoryListUri}?limit=10&offset=$offset&category_id=$categoryId');
  }
  Future<Response> addSubCategory(String subCategoryName, int? parenCategoryId, int? id, {bool isUpdate = false}) async {
    return await apiClient.postData(isUpdate?  AppConstants.updateSubCategoryUri : AppConstants.addSubCategoryUri,
  ...
  Future<Response> deleteCategory(int? categoryId) async {
    return await apiClient.getData('${AppConstants.deleteCategoryUri}?id=$categoryId');
  }
